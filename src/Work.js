...
const Work = () => {
  const data = workData.map((item, pos) => {
    ...
    return (
      <div className="bg-project-bg-light border border-gray-200 hover:border-gray-400 rounded-lg shadow dark:bg-project-bg-dark dark:border-gray-700" key={pos}>
        <a href={item.link} target="_blank">
          <img className="p-2 rounded-lg w-full" src={item.thumb} alt={item.name} />
          <div className="p-5">
